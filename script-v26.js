...
// V23 publication filters
const v23FilterButtons = document.querySelectorAll('.publication-tools .filter-btn');
const v23Entries = document.querySelectorAll('.publication-entry[data-type]');

v23FilterButtons.forEach(btn => {
  btn.addEventListener('click', () => {
    const filter = btn.dataset.filter;
    v23FilterButtons.forEach(b => b.classList.toggle('active', b === btn));

    v23Entries.forEach(entry => {
      const hidden = filter !== 'all' && entry.dataset.type !== filter;
      entry.classList.toggle('filtered-out', hidden);
    });
  });
});
...
// V26 copy email
document.querySelectorAll('.copy-email-v26').forEach(button => {
  button.addEventListener('click', async () => {
