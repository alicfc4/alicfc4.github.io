...
// V53 publication figure fallback
document.querySelectorAll('.publication-feature-figure-v53 .remote-site-image').forEach(img => {
  const fail = () => {
    const frame = img.closest('.publication-feature-figure-v53');
    if (frame) img.style.display = 'none';
  };
  img.addEventListener('error', fail, {once:true});
  if (img.complete && img.naturalWidth === 0) fail();
});
...
// V55 research figure fallback
document.querySelectorAll('.research-output-figure-v55 .remote-site-image').forEach(img => {
